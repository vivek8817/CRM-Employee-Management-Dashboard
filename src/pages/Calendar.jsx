...
export default function Calendar() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto w-full pb-10 flex flex-col xl:flex-row gap-6 relative h-full">
      ...
      {/* Main Calendar Area */}
       <div className="flex-1 flex flex-col bg-white border border-gray-100 rounded-[1.5rem] shadow-sm overflow-x-hidden overflow-y-visible">
        
        {/* 1. Calendar Header / Control Bar */}
        <div className="p-4 sm:p-6 border-b border-gray-100 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 z-20 bg-white relative">
          
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 sm:gap-4 ">
            <h2 className="text-sm sm:text-xl font-semibold tracking-tight text-gray-900 text-wrap text-balance">
              {MONTHS[currentDate.getMonth()]} {currentDate.getFullYear()}
            </h2>
            <div className="flex items-center bg-gray-50 rounded-full border border-gray-200 p-1">
              <button onClick={handlePrevMonth} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 hover:bg-white hover:shadow-sm hover:text-gray-900 transition-all">
                <iconify-icon icon="solar:alt-arrow-left-linear" width="20"></iconify-icon>
              </button>
              <button onClick={handleToday} className="px-4 h-8 rounded-full flex items-center justify-center text-xs font-semibold text-gray-600 hover:bg-white hover:shadow-sm hover:text-gray-900 transition-all">
                Today
              </button>
              <button onClick={handleNextMonth} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 hover:bg-white hover:shadow-sm hover:text-gray-900 transition-all">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full min-w-0 lg:w-auto lg:justify-end">
            ...
            <div className="hidden sm:flex items-center bg-gray-50 rounded-full p-1 border border-gray-100">
              <button className="px-4 py-1.5 rounded-full text-sm font-semibold bg-white text-gray-900 shadow-sm transition-all">Month</button>
              <button className="px-4 py-1.5 rounded-full text-sm font-semibold text-gray-500 hover:text-gray-900 transition-all opacity-50 cursor-not-allowed" disabled>Week</button>
            </div>

            {/* Department Filter */}
            <div className="relative w-full sm:w-auto sm:min-w-[180px] md:min-w-[210px]">
              <select className="w-full bg-white border border-gray-200 text-gray-700 text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-[#c5f82a]/50 hover:border-gray-300 transition-colors py-2 pl-4 pr-10 appearance-none font-medium">
              ...
              <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                <iconify-icon icon="solar:alt-arrow-down-linear" className="text-gray-400" width="16"></iconify-icon>
              </span>
            </div>

            {/* Search */}
            <div className="relative w-full sm:w-56 lg:w-64">
              <iconify-icon icon="solar:magnifer-linear" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" width="18"></iconify-icon>
              <input type="text" placeholder="Search employee..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-transparent rounded-full text-sm placeholder-gray-400 focus:bg-white focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#c5f82a]/50 transition-all" />
            </div>
          </div>
        </div>

        {/* 2. Calendar Grid */}
        <div className={clsx("flex-1 flex flex-col bg-gray-50 transition-opacity duration-200 p-0.5", isAnimating ? "opacity-0" : "opacity-100")}>
          
          {/* Weekday Headers */}
          <div className="grid grid-cols-7 gap-0.5 bg-gray-100/50 mb-0.5">
            {DAYS.map(day => (
              <div key={day} className="bg-white py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-400">
                {day}
              </div>
            ))}
          </div>
          ...
          {/* Days */}
          <div className="flex-1 grid grid-cols-7 grid-rows-6 gap-0.5 bg-gray-100/50">
            {days.map((dayObj, i) => {
              ...
              return (
                <div 
                  ...
                  className={clsx(
                    "group relative bg-white flex flex-col min-h-[100px] p-2 transition-all hover:z-30",
                    dayObj.isCurrentMonth ? "text-gray-900" : "text-gray-300 bg-white/50",
                    isToday && "ring-inset ring-2 ring-[#c5f82a] bg-[#c5f82a]/5",
                    isDisabled ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-gray-50 hover:shadow-[0_0_15px_rgba(0,0,0,0.05)]",
                    selectedDay?.date.toDateString() === dayObj.date.toDateString() && "ring-inset ring-2 ring-gray-900"
                  )}
                >
                  ...
                  {!isDisabled && dayObj.data && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[160px] bg-gray-900/95 backdrop-blur-sm text-white text-xs rounded-xl py-2 px-3 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 shadow-xl z-50">
                      <div className="font-semibold border-b border-gray-700 pb-1 mb-1">{dayObj.date.toDateString()}</div>
      ...
      <div className="w-full xl:w-80 flex flex-col gap-6 shrink-0">
        ...
        <div className="bg-white border border-gray-100 rounded-[1.5rem] p-6 shadow-sm">
          <h3 className="text-base font-semibold text-gray-900 mb-6 flex items-center gap-2">
            <iconify-icon icon="solar:graph-up-linear" className="text-[#c5f82a]" width="20"></iconify-icon>
            Monthly Insights
          </h3>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="text-3xl font-semibold tracking-tight text-gray-900">{insights.present}</div>
            </div>
            
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-50">
              <div>
                <div className="text-sm font-medium text-gray-500 mb-1">Absent Rate</div>
                <div className="text-xl font-semibold tracking-tight text-rose-500">{insights.absentRate}%</div>
              </div>
              <div>
                <div className="text-sm font-medium text-gray-500 mb-1">Leaves Taken</div>
                <div className="text-xl font-semibold tracking-tight text-amber-500">{insights.leave}</div>
              </div>
            </div>

            <div className="pt-4 border-t border-gray-50 flex items-start gap-3 bg-gray-50 p-4 rounded-2xl">
              <iconify-icon icon="solar:lightbulb-bolt-bold-duotone" className="text-amber-400 mt-0.5" width="24"></iconify-icon>
              <p className="text-sm text-gray-600 leading-relaxed">
                Mondays show a <span className="font-semibold text-gray-900">12% drop</span> in physical attendance compared to midweek.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Legend */}
        <div className="bg-white border border-gray-100 rounded-[1.5rem] p-6 shadow-sm">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400 mb-4">Legend</h3>
      ...
      {selectedDay && (
        <>
          ...
          />
          {/* Slide-over Panel */}
          <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white shadow-2xl z-[70] flex flex-col transform transition-transform duration-300 animate-[slideInRight_0.3s_ease-out]">
            <div className="px-6 py-6 border-b border-gray-100 flex items-center justify-between bg-[#fafafa]">
              <div>
                <h2 className="text-xl font-semibold tracking-tight text-gray-900">Daily Roster</h2>
                <p className="text-sm text-gray-500 mt-1">{selectedDay.date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>
              </div>
              <button 
                onClick={() => setSelectedDay(null)}
            ...
            <div className="grid grid-cols-4 gap-2 p-6 border-b border-gray-100">
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400 mb-4">Employee Status</h3>
              <div className="space-y-4">
                {dummyEmployees.map((emp, idx) => {
                  ...
                  return (
                    <div key={emp.id} className="flex items-center justify-between p-3 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                      <div className="flex items-center gap-3">
                        <img src={emp.avatar} alt={emp.name} className="w-10 h-10 rounded-full object-cover shadow-sm ring-2 ring-white" />
                        <div>
